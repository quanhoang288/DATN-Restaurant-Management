...
import { convertToOrderDTO, fromDTOToStateData } from '../../../utils/converter/convertToOrderDTO'
import { createOrder, getOrder, updateOrder, updateOrderItem } from '../../../apis/order'
...
function CancelAmountModal(props) {
  const { isModalVisible, handleCloseModal, cancelQuantity, handleChangeCancelQuantity, handleCancelOrderItem } = props
  return (
    <Modal isModalVisible={isModalVisible} title='Hủy món' handleCloseModal={handleCloseModal}>
      <div>
        <TextField
          label='Số lượng hủy'
        ...
        <div style={{ display: 'flex', float: 'right', marginTop: 10 }}>
          <Button variant='contained' color='primary' onClick={handleCancelOrderItem}>
            Lưu
          </Button>
...
function DiscountOptionModal(props) {
  ...
  return (
    <Modal title='Khuyến mãi trên hóa đơn' isModalVisible={isModalVisible} handleClose={handleCloseModal}>
      <TableContainer>
        <Table>
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {discountOptions.map((opt) => (
              <TableRow>
                <TableCell padding='checkbox'>
                  <Checkbox
                    checked={(selectedOptions || []).some((selected) => selected.id === opt.id)}
                    onChange={(e, checked) => handleSelectDiscountOption(opt, checked)}
                  />
                </TableCell>
                ...
                <TableCell>
                  {opt.method === 'invoice-discount' ? (
                    `${opt.constraint.discount_amount}${opt.constraint.discount_unit === 'cash' ? '' : '%'}`
                  ) : opt.method === 'invoice-giveaway' ? (
                    <Autocomplete
                      className='navbar__searchBar__container'
                      id='free-solo-2-demo'
                      options={opt.constraint.goods || []}
                      value={selectedOptions.find((selected) => selected.id === opt.id)?.discountItems || []}
                      onChange={(e, val) => handleUpdateDiscountItems(opt.id, val)}
              ...
              {(discounts || [])
                ...
                .map((discount) => (
                  <TableRow>
                    ...
                    <TableCell>
                      <IconButton onClick={() => handleRemoveDiscount(discount.id)}>
                        <ClearIcon />
                      </IconButton>
                    </TableCell>
                  </TableRow>
...
function OrderCreate({ orderId, isModalVisible, handleCloseModal }) {
  ...
  const [isCancelAmountModalVisible, setCancelAmountModalVisible] = useState(false)
  ...
  const handleAddItem = useCallback(
    (item) => {
      const itemPos = orderData.details.findIndex((orderItem) => orderItem.good_id === item.id)
      if (itemPos !== -1) {
        const details = orderData.details
        details[itemPos] = { ...details[itemPos], quantity: details[itemPos].quantity + 1 }
        setOrderData({ ...orderData, details })
      } else {
        setOrderData({ ...orderData, details: [...orderData.details, { good_id: item.id, name: item.name, quantity: 1 }] })
      }
    },
    [orderData]
  )
  ...
  const handleRemoveOrderItem = useCallback(
    (id) => {
      setOrderData({ ...orderData, details: orderData.details.filter((item) => item.good_id !== id) })
    },
    [orderData]
  )
  ...
  const handleUpdateServeStatus = useCallback(
    async (itemId) => {
      if (orderId) {
        ...
        setOrderData({ ...orderData, details: orderData.details.map((item) => (item.id === itemId ? { ...item, status: 'done' } : item)) })
      }
    },
    [orderData, orderId]
  )
  ...
  const handleCancelOrderItem = useCallback(async () => {
    if (orderId && itemIdToCancel) {
      const orderItem = orderData.details.find((item) => item.good_id === itemIdToCancel)
      await updateOrderItem(orderId, itemIdToCancel, { quantity: Math.max(orderItem.quantity - cancelQuantity, 0) })
  ...
  const handleSelectDiscountOption = useCallback(
    (option, checked) => {
      setOrderData({
        ...orderData,
        discounts: checked ? [...orderData.discounts, option] : orderData.discounts.filter((discount) => discount.id !== option.id),
      })
    },
    [orderData]
  )
  ...
  const handleUpdateDiscountItems = useCallback(
    (discountId, items) => {
      setOrderData({
        ...orderData,
        discounts: orderData.discounts.map((opt) => (opt.id === discountId ? { ...opt, discountItems: items } : opt)),
      })
    },
    [orderData]
  )
  ...
  return (
    <>
      ...
      />
      <Modal isModalVisible={isModalVisible} handleClose={handleCloseModal} title='Thêm đơn hàng'>
        <CustomTabs labels={['Thông tin đơn hàng', 'Gọi món']} activeTab={activeTab} onChangeActiveTab={(val) => setActiveTab(val)}>
          <TabPanel value={activeTab} index={0}>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', marginTop: 20, marginBottom: 20 }}>
                ...
                />
                <TextField label='Số điện thoại' InputLabelProps={{ shrink: true, style: { fontSize: 20 } }} style={{ marginLeft: 50 }} />
              </div>

              <FormControl margin='normal'>
                <FormLabel id='demo-radio-buttons-group-label'>Loại đơn hàng</FormLabel>
                <RadioGroup
                  row
                  aria-labelledby='demo-radio-buttons-group-label'
                  defaultValue='dine-in'
                  value={orderData.type}
                  onChange={(e, val) => setOrderData({ ...orderData, type: val })}
                  name='radio-buttons-group'
                >
                  <FormControlLabel value='dine-in' control={<Radio />} label='Ngồi tại bàn' />
                  <FormControlLabel value='delivery' control={<Radio />} label='Giao đi' />
                  <FormControlLabel value='takeaway' control={<Radio />} label='Mang về' />
                </RadioGroup>
              </FormControl>
              ...
              {orderData.type === 'dine-in' && (
                <Autocomplete
                  ...
                  value={tableOptions.find((opt) => opt.id === orderData.table) ?? null}
                  onChange={(e, val) => setOrderData({ ...orderData, table: val })}
                  getOptionLabel={(option) => option.name}
                  getOptionSelected={(option, value) => option.id === value.id}
                  filterOptions={(options, state) => options}
                  renderInput={(params) => (
              ...
              {orderData.type === 'delivery' && (
                <>
                  <TextField label='Địa chỉ giao hàng' variant='outlined' fullWidth margin='normal' />
                  <TextField label='Người giao hàng' variant='outlined' fullWidth margin='normal' />
                </>
              )}
          ...
          <TabPanel value={activeTab} index={1}>
            <div style={{ display: 'flex' }}>
              <div style={{ borderRight: '1px solid', minWidth: 400, padding: '1rem', marginRight: '1rem' }}>
                ...
                <div>
                  {goodItems.length > 0 ? (
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <Typography variant='h6'>Không tìm thấy món ăn</Typography>
                    </div>
                  )}
                </div>
              </div>
                ...
                <TableContainer>
                  <Table>
                    ...
                    <TableBody>
                              ...
                              onChange={(e) =>
                                setOrderData({
                                  ...
                                  details: orderData.details.map((orderItem) =>
                                    orderItem.good_id === item.good_id ? { ...orderItem, quantity: e.target.value } : orderItem
                                  ),
                                })
                              }
                            />
                          ...
                          {item.status !== 'done' && (
                            <TableCell width={50}>
                              <div style={{ display: 'flex' }}>
                                    ...
                                    <IconButton
                                      ...
                                      onClick={
                                        item.id
                                          ...
                                          : () => handleRemoveOrderItem(item.good_id)
                                      }
                                    >
                                      <ClearIcon />
                                    </IconButton>
                                ...
                                {item.status === 'ready_to_serve' && (
                                  <IconButton color='primary' onClick={() => handleUpdateServeStatus(item.good_id)}>
                      ...
                      {orderData.discounts
                        .filter((discount) => discount.discountItems && discount.discountItems.length > 0)
                        .reduce((items, discount) => items.concat(discount.discountItems), [])
                        .map((item) => (
                          <TableRow>
                            <TableCell width={250}>
                              <div style={{ display: 'flex', alignItems: 'center' }}>
                                <Typography style={{ marginRight: 10 }}>{item.name}</Typography>
                                <ChipLabel label='KM' variant='failure' />
                              </div>
                            </TableCell>
                            <TableCell align='center' width={100}>
                              <TextField type='number' value={item.quantity || 1} disabled />
                            </TableCell>
                            <TableCell width={150} align='center'>
                              {item.finished_quantity || 0}
                            </TableCell>
                    ...
                    <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls='panel1a-content' id='panel1a-header'>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <Tooltip title='Xem chương trình khuyến mãi khả dụng'>
                          <IconButton style={{ color: '#FFB6C1' }} onClick={() => setDiscountModalVisible(true)}>
                            <RedeemIcon />
                          </IconButton>
                        </Tooltip>
                      </div>
                    ...
                    <AccordionDetails>
                      <OrderDiscounts
                        ...
                        handleRemoveDiscount={(id) =>
                          setOrderData({ ...orderData, discounts: orderData.discounts.filter((discount) => discount.id !== id) })
                        }
                      />
                    </AccordionDetails>
                  </Accordion>
        ...
        <div style={{ display: 'flex', float: 'right', marginTop: 2 }}>
          <Button variant='contained' onClick={() => setPaymentModalVisible(true)}>
            Lưu và thanh toán
          </Button>
