...
import { createReservation, getReservation, updateReservation } from '../../../apis/reservation'
...
import { convertToReservationDTO, fromDTOToStateData } from '../../../utils/converter/convertToReservationDTO'
...
function ReservationCreate({ reservationId, isModalVisible, handleCloseModal }) {
  ...
  const handleToggleTable = useCallback(
    (table) => {
      const isAvailable = !(table.reservations || []).some((res) => ['pending', 'confirmed', 'serving'].includes(res.status))
      if (isAvailable) {
        const isSelected = reservationData.tables.some((t) => table.id === t.id)
        if (!isSelected) {
          setReservationData({ ...reservationData, tables: [...reservationData.tables, table] })
        } else {
          setReservationData({ ...reservationData, tables: reservationData.tables.filter((t) => t.id !== table.id) })
        }
      }
    },
    [reservationData]
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={reservationId ? 'Sửa thông tin đặt bàn' : 'Thêm thông tin đặt bàn'}
      >
        <div className='reservation__create__container'>
          <CustomTabs labels={['Thông tin chung', 'Xếp bàn']} activeTab={activeTab} onChangeActiveTab={(val) => setActiveTab(val)}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', marginTop: 20, marginBottom: 20 }}>
                  <Autocomplete
                    ...
                    onChange={(e, val) => setReservationData({ ...reservationData, customerName: val })}
                ...
                <FormControl margin='normal'>
                  <FormLabel id='demo-radio-buttons-group-label'>Trạng thái đặt bàn</FormLabel>
                  <RadioGroup
                    row
                  ...
                  >
                    <FormControlLabel value='pending' control={<Radio />} label='Chờ xác nhận' />
                    <FormControlLabel value='confirmed' control={<Radio />} label='Đã xác nhận (Chờ nhận bàn)' />
                    <FormControlLabel value='serving' control={<Radio />} label='Đã nhận bàn' />
                  </RadioGroup>
                </FormControl>
                <TextField
            ...
            <TabPanel value={activeTab} index={1}>
              <div>
                <div className='search__container' style={{ display: 'flex', alignItems: 'center' }}>
                  <TextField
                    label='Xếp vào bàn'
                    margin='normal'
                    InputLabelProps={{
                  ...
                  {reservationData.tables.length > 0 && (
                    <Typography>{`Bàn đang chọn: ${reservationData.tables.map((table) => table.name).join(', ')}`}</Typography>
                  )}
                </div>
                <div style={{ display: 'flex', marginTop: 20, marginBottom: 20 }}>
                  <div style={{ display: 'flex', marginRight: 50 }}>
                    <Typography>Toàn bộ nhà hàng: Trống 38/38 bàn</Typography>
                    <Typography>{`Tầng 2: Trống ${
                      tableOptions.filter(
                        (opt) => !opt.reservations.some((res) => ['pending', 'confirmed', 'serving'].includes(res.status))
                      ).length
                    }/${tableOptions.length} bàn`}</Typography>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', marginRight: '1rem' }}>
                      <div style={{ height: 20, width: 20, backgroundColor: 'grey', marginRight: 5 }}></div>
                      <Typography>Bàn trống</Typography>
                    </div>
                    <div style={{ display: 'flex', marginRight: '1rem' }}>
                      <div style={{ height: 20, width: 20, backgroundColor: 'blue', marginRight: 5 }}></div>
                      <Typography>Bàn đang phục vụ</Typography>
                    </div>
                    <div style={{ display: 'flex' }}>
                      <div style={{ height: 20, width: 20, backgroundColor: 'orange', marginRight: 5 }}></div>
                ...
                <div style={{ display: 'flex', marginBottom: 20 }}>
                  ...
                  >
                    {tableOptions.map((opt) =>
                      reservationData.tables.some((table) => table.id === opt.id) ? (
                        <Badge color='primary' badgeContent={reservationData.tables.findIndex((table) => table.id === opt.id) + 1}>
                          <div
                            className={`table__item ${
                              opt.reservations.some((res) => ['pending', 'confirmed'].includes(res.status))
                                ? 'table__item__reserved'
                                : opt.reservations.some((res) => res.status === 'serving')
                                ? 'table__item__used'
          ...
          <div style={{ display: 'flex', float: 'right', marginTop: 2 }}>
            <Button variant='contained' color='primary' onClick={handleSaveReservation}>
              Lưu
            </Button>
