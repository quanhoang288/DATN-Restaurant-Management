...
import { getOrder, getOrders, updateOrder, updateOrderItem } from '../../../apis/order'
...
function OrderItem(props) {
  ...
  return (
    <Card className='order__item__card' style={{ height: '100%', marginBottom: '1.5rem' }} onClick={handleViewDetail}>
      <CardHeader
        className='order__item__card__header'
        title={order.reservation_table_id}
...
function FinishQuantityModal(props) {
  const { isModalVisible, handleCloseModal, finishedQuantity, handleChangeFinishedQuantity, handleFinishOrderItem } = props
  return (
    <Modal title='Hoàn thành món' isModalVisible={isModalVisible} handleCloseModal={handleCloseModal}>
      <TextField
        fullWidth
      ...
      <div style={{ display: 'flex', float: 'right', marginTop: 2 }}>
        <Button variant='contained' color='primary' onClick={handleFinishOrderItem}>
          Lưu
        </Button>
...
function OrderDetail(props) {
  ...
  const [isFinishQuantityModalVisible, setFinishQuantityModalVisible] = useState(false)
  ...
  const updateStatus = (orderId, curStatus) => {
    const newStatus = curStatus === 'pending' ? 'in_progress' : curStatus === 'in_progress' ? 'done' : 'pending'
    ...
    async (itemId, newStatus, finishedQuantity = null) => {
      ...
      setOrderDetail({
        ...
        goods: (orderDetail.goods || []).map((item) =>
          item.id === itemId ? { ...item, OrderDetail: { ...item.OrderDetail, ...updateData } } : item
        ),
      })
    },
    [orderId, orderDetail]
  ...
  const handleRejectOrderItem = useCallback(
    async (itemId, rejectForAllOrders = false) => {
      ...
      setOrderDetail({
        ...orderDetail,
        goods: (orderDetail.goods || []).map((item) =>
          item.id === itemId ? { ...item, OrderDetail: { ...item.OrderDetail, status: 'rejected' } } : item
        ),
      })
  ...
  return (
    <>
      ...
      <FinishQuantityModal
        ...
        handleChangeFinishedQuantity={(newQuantity) => setFinishedQuantity(newQuantity)}
        handleFinishOrderItem={() =>
          updateOrderItem(orderId, orderItemIdToFinish, { status: 'ready_to_serve', finished_quantity: finishedQuantity })
        }
      />
      <Card style={{ height: '100%', marginBottom: '1.5rem' }}>
        <CardHeader
          ...
          subheader={orderDetail.created_at ?? 'Bàn 1 - 10:30'}
          action={
        ...
        />
        <CardContent style={{ display: 'flex', height: '80%', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div>
            <List component='div' style={{ marginBottom: 20, maxHeight: 400, overflowY: 'auto' }}>
              {(orderDetail.goods || []).map((item) => (
                <ListItem>
                  {['ready_to_serve', 'done'].includes(item.OrderDetail.status) && (
                  ...
                  <ListItemText
                    primary={item.name}
                    className={item.OrderDetail.status === 'rejected' ? 'line__through' : ''}
                          ...
                          <>
                            {orderDetail.prepare_status === 'in_progress' && (
                              <Tooltip title='Bắt đầu chế biến'>
                                <IconButton onClick={() => handleUpdateOrderItemStatus(item.id, 'in_progress')}>
                                  <AlarmOnIcon />
                                </IconButton>
                              </Tooltip>
                            )}
            ...
            <TextField value={orderDetail.note ?? ''} variant='outlined' fullWidth disabled multiline rows={4} label='Ghi chú' />
          </div>

          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '3rem' }}>
            <Button color='secondary'>Ngừng nhận đơn</Button>
            <Button color='primary' onClick={() => updateStatus(orderId, orderDetail.prepare_status)}>
  ...
  return (
    <Card style={{ height: '100%' }}>
      ...
      <CardContent>
        <CardContent>
          {(orders || []).map((order) => (
            <OrderItem order={order} handleViewDetail={() => handleViewDetail(order.id)} />
          ))}
        </CardContent>
      </CardContent>
    </Card>
...
function KitchenDisplay(props) {
  ...
  const handleUpdateOrderStatus = useCallback(
    async (orderId, newStatus) => {
      ...
      setOrders(orders.map((order) => (order.id === orderId ? { ...order, prepare_status: newStatus } : order)))
    },
    [orders]
  )
  ...
  return (
    <Main>
      <CustomTabs labels={['Theo bàn', 'Theo món']} activeTab={activeTab} onChangeActiveTab={(val) => setActiveTab(val)}>
            ...
            <Grid container spacing={2} style={{ height: '80vh' }}>
              <Grid item style={{ background: '#f5f5f5' }} xs={4}>
                <OrderGroup
                  title='Chờ chế biến'
                  type='todo'
                  orders={orders.filter((order) => order.prepare_status === 'pending')}
              ...
              <Grid item style={{ background: '#f5f5f5' }} xs={4}>
                <OrderGroup
                  title='Đang chế biến'
                  type='in-progress'
                  orders={orders.filter((order) => order.prepare_status === 'in_progress')}
                  handleViewDetail={(orderId) => setSelectedOrder(orderId)}
                />{' '}
              </Grid>
        ...
        <TabPanel value={activeTab} index={1}>
          <List style={{ flex: 1 }}>
            <ListItem style={{ justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', flex: 1, minWidth: 500, alignItems: 'center' }}>
                <Typography style={{ marginRight: 100, fontWeight: 'bold', fontSize: 18 }}>Bo xao xa ot</Typography>
